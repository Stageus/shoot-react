...
const HeaderAlarm = () => {
    ...
    const alarmContent = alarmList.map((element, Idx) => {
        return (
            <Div pointer key={`alarmBox_${Idx}`} position="relative" width="100%" margin="0 0 20px 0">
                <Div>
                    <P fontSize="sm">
                        {
    ...
    return (
        <Div ref={alarmRef} position="relative" width="24px" height="24px" margin="0 15px 0 0">
            <Div pointer width="24px">
                <Img onClick={openAlarmContainerEvent} src="/assets/images/bellOn.svg"/>
            </Div>
            <Div display={alarmOpen === true ? "block" : "none"} width="245px" position="absolute" bottom="0" right="0" transform="translate(0, 100%)" backgroundColor="white" borderRadius="5px" shadow="0 4px 4px 0 rgba(0,0,0,0.35)">
